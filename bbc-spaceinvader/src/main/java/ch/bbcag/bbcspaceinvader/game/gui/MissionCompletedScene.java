...
public class MissionCompletedScene extends Scene implements IInitializable {

	private static Group group = new Group();
	
	public MissionCompletedScene(Navigator navigator) {
		super(group);

		Canvas canvas = new Canvas(Const.SCREEN_WIDTH, Const.SCREEN_HEIGHT);
		GraphicsContext gc = canvas.getGraphicsContext2D();
		Image image = new Image(this.getClass().getResourceAsStream("/BackgroundMissionCompleted.png"));
	...
	public void initialize() {
		Sound.start(EnumSounds.MISSION_COMPLETED);
   }
}
