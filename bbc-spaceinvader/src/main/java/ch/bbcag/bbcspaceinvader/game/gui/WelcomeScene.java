...
public class WelcomeScene extends Scene implements IInitializable {
	...
	public WelcomeScene(Navigator navigator) {
		...
		Image image = new Image(this.getClass().getResourceAsStream("/BackgroundStart.png"));
		gc.drawImage(image, 0, 0);
		group.getChildren().add(canvas);

		this.setOnKeyPressed((e) -> { 
			if (e.getCode() == KeyCode.SPACE){
				navigator.goTo(EnumScenes.GAME);
			}
		});
	}
